...
def train(
    ...
    console.print(
        f"[bold green]Initialization of the pipeline with config:[/bold green] {config_name}"
    )
...
def resolve_checkpoint_path(checkpoint_ref: str, tracking_uri: str) -> str:
    ...
    if checkpoint_ref.startswith(mlflow_prefixes):
        console.print(
            "[bold yellow]Attempt to download an artefact form MLflow/DagsHub:[/bold yellow] "
            "{checkpoint_ref}"
        )
        ...
        try:
            ...
            raise FileNotFoundError(f"Failed to download from MLflow: {checkpoint_ref}") from e
...
def test(
    ctx: typer.Context,
    checkpoint_ref: str = typer.Argument(
        ...,
        help="Local path (e.g. best_model.ckpt) or MLflow URI "
        "(e.g. runs:/<run_id>/checkpoints/best_model.ckpt)",
    ),
    config_name: str = typer.Option(
    ...
    tracking_uri = cfg.trainer.get("logger", {}).get(
